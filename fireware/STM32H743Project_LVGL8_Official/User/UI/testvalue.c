                        ...
                        float measured_value)
{
    ...
    /* 2. 更新圆弧中央文字，例如"20%"。 */
    /*
     * 官方LVGL8工程默认关闭LV_SPRINTF_USE_FLOAT，因此不能使用"%.0f%%"。
     * 先将百分比转换为整数，再用整数格式输出，确保末尾的百分号可以正常显示。
     */
    lv_label_set_text_fmt(test_struct->label_value, "%d%%", (int32_t)(percent + 0.5f));
                               ...
                               float measured_value)
{
    ...
    *test_struct = (testvalue_st){0};
    ...
    lv_obj_clear_flag(test_struct->container, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_clear_flag(test_struct->arc_value, LV_OBJ_FLAG_CLICKABLE);
     ...
     */
    lv_obj_set_style_arc_width(test_struct->arc_value, 14, LV_PART_MAIN);
    ...
    lv_obj_set_style_arc_width(test_struct->arc_value, 14, LV_PART_INDICATOR);
    ...
    /* 固定宽度并居中，避免数值从一位变成三位时百分号被裁掉。 */
    lv_obj_set_width(test_struct->label_value, 80);
    lv_obj_set_style_text_align(test_struct->label_value, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
