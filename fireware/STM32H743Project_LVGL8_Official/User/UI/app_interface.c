...
{
    return sensor < APP_SENSOR_COUNT;
}
...
static void app_interface_sync_control_widgets(void)
{
    ...
    if(g_setting_ui.fan_switch != NULL) {
        if(fan_enabled) {
            ...
            lv_obj_clear_state(g_setting_ui.fan_switch, LV_STATE_CHECKED);
        }
    }
    ...
    if(g_setting_ui.window_switch != NULL) {
        if(window_open) {
            ...
            lv_obj_clear_state(g_setting_ui.window_switch, LV_STATE_CHECKED);
        }
    }
