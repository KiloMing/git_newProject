 ...
 * @attention
 *
 * Platform: STM32H743 development board`r`n *
 ****************************************************************************************************
 */
...
static lv_obj_t *label_left;            /* 左侧百分比标�?*/
static lv_obj_t *label_right;           /* 右侧百分比标�?*/
...
/**
 * @brief  圆弧事件回调
 * @param  *e ：事件相关参数的集合，它包含了该事件的所有数�?
 * @return �?
 */
static void arc_event_cb(lv_event_t *e)
{
    lv_obj_t *target = lv_event_get_target(e);                                  /* 获取触发�?*/
    lv_event_code_t code = lv_event_get_code(e);                                /* 获取事件类型 */

    if(code == LV_EVENT_VALUE_CHANGED)
    {
        lv_label_set_text_fmt(label_left, "%d%%", lv_arc_get_value(target));    /* 获取当前值，更新左侧百分比标�?*/
        lv_label_set_text_fmt(label_right, "%d%%", lv_arc_get_value(target));   /* 获取当前值，更新右侧百分比标�?*/
        lv_arc_set_value(arc_right, lv_arc_get_value(target));                  /* 获取左侧圆弧当前值，更新右侧圆弧当前�?*/
    }
}
...
void lv_example_arc1(void)
{
    ...
    lv_arc_set_value(arc_left, 0);                                              /* 设置当前�?*/
    lv_obj_set_style_arc_width(arc_left, arc_width, LV_PART_MAIN);              /* 设置背景弧宽�?*/
    lv_obj_set_style_arc_width(arc_left, arc_width, LV_PART_INDICATOR);         /* 设置前景弧宽�?*/
    lv_obj_add_event_cb(arc_left, arc_event_cb, LV_EVENT_VALUE_CHANGED, NULL);  /* 添加事件 */

    /* 左侧百分比标�?*/
    label_left = lv_label_create(lv_scr_act());                                 /* 创建百分比标�?*/
...
void lv_example_arc2(void)
{
    ...
    lv_arc_set_value(arc_right, 0);                                             /* 设置当前�?*/
    lv_arc_set_bg_angles(arc_right, 0, 360);                                    /* 设置背景弧角�?*/
    lv_arc_set_rotation(arc_right, 270);                                        /* 设置旋转角度 */
    lv_obj_remove_style(arc_right, NULL, LV_PART_KNOB);                         /* 去除旋钮 */
    lv_obj_clear_flag(arc_right, LV_OBJ_FLAG_CLICKABLE);                        /* 去除可点击属�?*/
    lv_obj_set_style_arc_width(arc_right, arc_width, LV_PART_MAIN);             /* 设置背景弧宽�?*/
    lv_obj_set_style_arc_width(arc_right, arc_width, LV_PART_INDICATOR);        /* 设置前景弧宽�?*/

    /* 右侧百分比标�?*/
    label_right = lv_label_create(lv_scr_act());                                /* 创建百分比标�?*/
