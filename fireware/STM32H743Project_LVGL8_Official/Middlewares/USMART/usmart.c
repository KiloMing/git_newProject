 ...
 * @brief       USMART 串口调试组件
 *
 *              USMART是一个串口调试交互组件,通过 它,你可以通过串口助手调用程
 ...
 *
 * @note        USMART是一个串口调试交互组件,通过 它,你可以通过串口助手调用程
 *              序里面的任何函数,并执行.因此,你可以随意更改函数的输入参数(支持数字(10/16进制,支持负数)
 *              、字符串、函数入口地址等作为参数),单个函数最多支持10个输入参数,并支持函数返 回值显示.
 *              V2.1版本以后新增hex和dec两个指令.他们可以用于设置函数参数的显示格式.也可以用于数据的进
 *              制转换,例如:
...
uint8_t usmart_sys_cmd_exe(char *str)
{
    ...
    switch (i)
    {
        ...
        case 1: /* 帮助指令 */
            ...
            USMART_PRINTF("    USMART是一个串口调试交互组件,通过 \r\n");
 ...
 *              若未使用本工程默认串口接收实现,则USART_RX_STA和USART_RX_BUF[]需要用户自行实现
 *
 * @param       无
 * @retval      无
 */
