...
void BlueTeeth_Init(void){
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_USARTx, ENABLE);          //不要忘记打开串口时钟
    RCC_APB2PeriphClockCmd(BlueTeeth_GPIO_CLK, ENABLE );	        //开启GPIO时钟
    GPIO_InitTypeDef GPIO_InitStructure;
    GPIO_InitStructure.GPIO_Pin = BlueTeeth_Pin_Tx;					//配置Tx引脚
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;		            //设置为推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(BlueTeeth_GPIOx, &GPIO_InitStructure);				// 初始化Tx引脚
    
    GPIO_InitStructure.GPIO_Pin = BlueTeeth_Pin_Rx;					// 配置Rx引脚
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;		            // 配置为上拉输入
    GPIO_Init(BlueTeeth_GPIOx, &GPIO_InitStructure);				// 初始化Rx
